...
export async function getQuoteOfDay() {
  ...
  try {
    ...
    if (!notes.length) {
      const fallback = await db.prepare(`SELECT id, slug, title, quote_of_day FROM books WHERE published = 1 AND TRIM(quote_of_day) != '' ORDER BY updated_at DESC LIMIT 1`).first();
      return fallback ? {
        id: `book-${fallback.id}`,
