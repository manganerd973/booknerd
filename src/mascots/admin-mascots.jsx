...
import { orderedDraftLines, secondSpeaker } from './admin-dialogue-order.js';
...
const newReaderWelcomeTemplate = {
  ...
  pages: ['home'],
  firstSpeaker: 'till',
  tillText: 'О, новый читатель! Мне сразу показать Вам лучшие книги?\nЯ не мешаю. Я создаю гостеприимную атмосферу.',
  ivanText: 'Сначала позволь человеку осмотреться.\nОчень громкую гостеприимную атмосферу.',
};
...
export default function AdminMascots({ onNotice }) {
  ...
  const editDialogue = (dialogue) => {
    setDraft({
      ...
      ivanText: (dialogue.lines || []).filter((line) => line.character === 'ivan').map((line) => line.text).join('\n'),
      tillText: (dialogue.lines || []).filter((line) => line.character === 'till').map((line) => line.text).join('\n'),
  ...
  return (
    <section className="admin-content admin-mascots-page">
      ...
      <section className="admin-mascot-config">
        ...
        <label className="admin-switch-row"><span><strong>AI-ответы</strong><small>Создают короткие новые реакции только после вопроса читателя. Подтверждённый ответ BOOKNERD и защита от спойлеров сохраняются.</small></span><input type="checkbox" checked={config.aiEnabled} onChange={(event) => setConfig({ ...config, aiEnabled: event.target.checked })} /></label>
      ...
      <form className="admin-mascot-dialogue-editor" id="admin-mascot-dialogue-editor" onSubmit={saveDialogue}>
        <header>
          <div><span>{draft.id ? 'РЕДАКТИРОВАНИЕ' : 'НОВАЯ РЕПЛИКА'}</span><h2>{draft.id ? 'Изменить сценку' : 'Добавить сценку'}</h2></div>
          <div className="admin-mascot-editor-actions">
            {!draft.id ? <button type="button" onClick={() => { setDraft(newReaderWelcomeTemplate); setPreviewOpen(true); }}><Sparkles size={17} /> Приветствие нового читателя</button> : null}
            {draft.id ? <button type="button" onClick={() => setDraft(blankDialogue)}><X size={17} /> Отмена</button> : null}
          </div>
        </header>
        <div className="admin-fields three-columns">
          ...
          <label><span>Первым говорит</span><select value={draft.firstSpeaker} onChange={(event) => setDraft({ ...draft, firstSpeaker: event.target.value === 'ivan' ? 'ivan' : 'till' })}><option value="till">Тилл</option><option value="ivan">Иван</option></select><small>Вы задаёте порядок только для этого диалога.</small></label>
          <label><span>Вторым говорит</span><select value={secondSpeaker(draft.firstSpeaker)} onChange={(event) => setDraft({ ...draft, firstSpeaker: event.target.value === 'ivan' ? 'till' : 'ivan' })}><option value="ivan">Иван</option><option value="till">Тилл</option></select><small>При изменении порядок реплик перестроится автоматически.</small></label>
        </div>
        ...
        <div className="admin-mascot-line-fields">
          <label className="is-ivan"><span><img src="/mascots/ivan.webp" alt="" /> Реплики Ивана — каждая с новой строки</span><textarea rows="4" value={draft.ivanText} onChange={(event) => setDraft({ ...draft, ivanText: event.target.value })} maxLength={1600} /></label>
          <label className="is-till"><span><img src="/mascots/till.webp" alt="" /> Реплики Тилла — каждая с новой строки</span><textarea rows="4" value={draft.tillText} onChange={(event) => setDraft({ ...draft, tillText: event.target.value })} maxLength={1600} /></label>
        </div>
        {previewOpen && (draft.ivanText.trim() || draft.tillText.trim()) ? (
          <div className="admin-mascot-preview" aria-label="Предпросмотр реплики">
            {orderedDraftLines(draft).map((line, index) => line.character === 'ivan'
              ? <div className="is-ivan" key={`${line.character}-${index}`}><img src="/mascots/ivan.webp" alt="Иван" /><p><strong>Иван</strong>{line.text}</p></div>
              : <div className="is-till" key={`${line.character}-${index}`}><p><strong>Тилл</strong>{line.text}</p><img src="/mascots/till.webp" alt="Тилл" /></div>)}
          </div>
        ) : null}
