...
export default function AdvancedSearchPage({ books = [] }) {
  ...
  }, [books, chapters, country, genre, length, query]);

  return (
    <div className="site-shell inner-site-shell advanced-search-page">
      <SiteHeader active="search" />
      <main>
        <section className="inner-hero"><span className="section-number">ПОИСК ПО БИБЛИОТЕКЕ</span><div><h1>Найдите<br /><em>свою историю.</em></h1><p>Ищите книгу по названию, автору, жанру, стране, переводчику или длине.</p></div></section>
        <section className="advanced-search-content">
          <div className="advanced-search-query"><Search size={24} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Название, автор или жанр…" autoFocus /><span>{results.length}</span></div>
          <div className="advanced-search-filters"><SlidersHorizontal size={20} /><label><span>Жанр</span><select value={genre} onChange={(event) => setGenre(event.target.value)}><option value="">Все</option>{genres.map((item) => <option key={item}>{item}</option>)}</select></label><label><span>Страна</span><select value={country} onChange={(event) => setCountry(event.target.value)}><option value="">Все</option>{countries.map((item) => <option key={item}>{item}</option>)}</select></label><label><span>Длина</span><select value={length} onChange={(event) => setLength(event.target.value)}><option value="all">Любая</option><option value="short">До 300 стр.</option><option value="medium">301–500 стр.</option><option value="long">Больше 500 стр.</option></select></label><label><span>Глав</span><select value={chapters} onChange={(event) => setChapters(event.target.value)}><option value="all">Любое число</option><option value="short">До 20</option><option value="medium">21–50</option><option value="long">51+</option></select></label></div>
          <div className="advanced-search-view"><Grid3X3 size={17} /> Плитка</div>
          <div className="advanced-search-results">{results.map((book) => <article key={book.id}><a className="advanced-search-cover" href={`/books/${book.slug}`}>{book.coverUrl ? <img src={book.coverUrl} alt={`Обложка книги «${book.title}»`} /> : <span>B</span>}</a><div><small>{[book.country, `${book.publishedChapterCount || 0} глав`].filter(Boolean).join(' · ')}</small><h2><a href={`/books/${book.slug}`}>{book.title}</a></h2><p>{book.author}</p><div>{(book.tropes || []).slice(0, 3).map((trope) => <span key={trope}>{trope}</span>)}</div><em>{[book.translator && `Перевод: ${book.translator}`, book.editor && `Редактура: ${book.editor}`].filter(Boolean).join(' · ')}</em></div></article>)}</div>
        </section>
      </main>
      <SiteFooter />
    </div>
