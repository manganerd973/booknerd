...
import { mapWorldMap } from './world-map.js';
...
export function mapBook(row) {
  if (!row) return null;
  const genres = normalizeGenres(parseList(row.genres));
  return {
    id: row.id,
    slug: row.slug,
    title: row.title,
    ...
    releaseDays: parseList(row.release_days),
    author: row.author,
    country: row.country || '',
    ...
    suitabilityProfile: parseObject(row.suitability_profile),
    ageRating: row.age_rating || '',
    ageReason: row.age_reason || '',
    hasHotScenes: Boolean(row.has_hot_scenes),
    hotSceneChapters: row.hot_scene_chapters || '',
    synopsis: row.synopsis || '',
    genres,
    genre: genres[0] || 'Другое',
...
export async function listPublicBooks() {
  ...
  return cachedRead('public-books', 30_000, async () => {
