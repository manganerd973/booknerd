...
export async function ensureDb() {
  ...
  if (!schemaPromise) {
    schemaPromise = (async () => {
      try {
        await db.batch([
          db.prepare(`SELECT mascot_preferences FROM reader_profiles LIMIT 1`),
          db.prepare(`SELECT photo_key FROM reader_profiles LIMIT 1`),
          db.prepare(`SELECT id FROM mascot_settings LIMIT 1`),
          db.prepare(`SELECT id FROM mascot_dialogues LIMIT 1`),
          db.prepare(`SELECT workflow_status, scheduled_at FROM chapters LIMIT 1`),
          db.prepare(`SELECT last_page, reading_seconds, last_opened_at FROM reader_library LIMIT 1`),
          db.prepare(`SELECT hidden_at FROM reader_notifications LIMIT 1`),
        ]);
        return;
      } catch (error) {
      ...
      await db.batch([
        ...
        planned_chapter_count INTEGER DEFAULT 0 NOT NULL,
        author_birthday TEXT DEFAULT '' NOT NULL,
        original_release_date TEXT DEFAULT '' NOT NULL,
        translator TEXT DEFAULT '' NOT NULL,
        editor TEXT DEFAULT '' NOT NULL,
        proofreader TEXT DEFAULT '' NOT NULL,
        playlist_url TEXT DEFAULT '' NOT NULL,
        team_pick INTEGER DEFAULT 0 NOT NULL,
        quote_of_day TEXT DEFAULT '' NOT NULL,
        search_aliases TEXT DEFAULT '[]' NOT NULL,
        dedication TEXT DEFAULT '' NOT NULL,
        trigger_warnings TEXT DEFAULT '[]' NOT NULL,
        suitability_profile TEXT DEFAULT '{}' NOT NULL,
        age_rating TEXT DEFAULT '' NOT NULL,
        age_reason TEXT DEFAULT '' NOT NULL,
        ...
        status TEXT DEFAULT 'Анонс' NOT NULL,
        progress INTEGER DEFAULT 0 NOT NULL,
        cover_key TEXT,
        world_map_key TEXT,
        world_map_name TEXT DEFAULT '' NOT NULL,
        world_map_content_type TEXT DEFAULT '' NOT NULL,
        world_map_size_bytes INTEGER DEFAULT 0 NOT NULL,
        world_map_markers TEXT DEFAULT '[]' NOT NULL,
        ...
        db.prepare(`CREATE TABLE IF NOT EXISTS chapter_music (
        ...
        db.prepare(`CREATE UNIQUE INDEX IF NOT EXISTS chapter_music_storage_key_unique ON chapter_music (storage_key)`),
        ...
        book_id TEXT NOT NULL,
        chapter_id TEXT,
        context TEXT DEFAULT 'comments' NOT NULL,
        parent_id TEXT,
        visitor_key TEXT DEFAULT '' NOT NULL,
        author_role TEXT DEFAULT 'reader' NOT NULL,
        ...
        db.prepare(`CREATE TABLE IF NOT EXISTS reader_notifications (
        id TEXT PRIMARY KEY NOT NULL,
        visitor_key TEXT NOT NULL,
        ...
        db.prepare(`CREATE UNIQUE INDEX IF NOT EXISTS reader_notifications_visitor_event_unique ON reader_notifications (visitor_key, event_key)`),
        db.prepare(`CREATE INDEX IF NOT EXISTS reader_notifications_visitor_created_idx ON reader_notifications (visitor_key, created_at)`),
        db.prepare(`CREATE INDEX IF NOT EXISTS reader_notifications_visitor_read_idx ON reader_notifications (visitor_key, read_at, created_at)`),
        ...
        db.prepare(`CREATE TABLE IF NOT EXISTS reader_profiles (
        visitor_key TEXT PRIMARY KEY NOT NULL,
        display_name TEXT DEFAULT 'Читатель BOOKNERD' NOT NULL,
        photo_key TEXT,
        photo_name TEXT DEFAULT '' NOT NULL,
        photo_content_type TEXT DEFAULT '' NOT NULL,
        photo_size_bytes INTEGER DEFAULT 0 NOT NULL,
        ...
        mascot_preferences TEXT DEFAULT '{}' NOT NULL,
        created_at TEXT NOT NULL,
        updated_at TEXT NOT NULL
        )`),
        db.prepare(`CREATE TABLE IF NOT EXISTS mascot_settings (
        id TEXT PRIMARY KEY NOT NULL,
        enabled INTEGER DEFAULT 1 NOT NULL,
        ai_enabled INTEGER DEFAULT 0 NOT NULL,
        disabled_pages TEXT DEFAULT '[]' NOT NULL,
        blocked_topics TEXT DEFAULT '[]' NOT NULL,
        updated_at TEXT NOT NULL,
        updated_by TEXT DEFAULT '' NOT NULL
        )`),
        db.prepare(`CREATE TABLE IF NOT EXISTS mascot_dialogues (
        id TEXT PRIMARY KEY NOT NULL,
        ...
        db.prepare(`CREATE INDEX IF NOT EXISTS mascot_dialogues_active_dates_idx ON mascot_dialogues (active, starts_at, ends_at)`),
        db.prepare(`CREATE TABLE IF NOT EXISTS paragraph_reactions (
        id TEXT PRIMARY KEY NOT NULL,
        visitor_key TEXT NOT NULL,
        book_id TEXT NOT NULL,
        chapter_id TEXT NOT NULL,
        ...
        db.prepare(`CREATE UNIQUE INDEX IF NOT EXISTS paragraph_reactions_reader_unique ON paragraph_reactions (visitor_key, chapter_id, paragraph_index, emoji)`),
        db.prepare(`CREATE INDEX IF NOT EXISTS paragraph_reactions_chapter_idx ON paragraph_reactions (chapter_id, paragraph_index)`),
        db.prepare(`CREATE TABLE IF NOT EXISTS reader_dictionary (
        id TEXT PRIMARY KEY NOT NULL,
        visitor_key TEXT NOT NULL,
        book_id TEXT NOT NULL,
        chapter_id TEXT NOT NULL,
        ...
        db.prepare(`CREATE INDEX IF NOT EXISTS reader_dictionary_visitor_book_idx ON reader_dictionary (visitor_key, book_id, created_at)`),
        db.prepare(`CREATE TABLE IF NOT EXISTS reader_public_notes (
        id TEXT PRIMARY KEY NOT NULL,
        visitor_key TEXT NOT NULL,
        ...
        db.prepare(`CREATE UNIQUE INDEX IF NOT EXISTS reader_public_notes_source_unique ON reader_public_notes (visitor_key, source_annotation_id)`),
        db.prepare(`CREATE INDEX IF NOT EXISTS reader_public_notes_status_created_idx ON reader_public_notes (status, created_at)`),
        db.prepare(`CREATE INDEX IF NOT EXISTS reader_public_notes_pinned_updated_idx ON reader_public_notes (is_pinned, updated_at)`),
        db.prepare(`CREATE INDEX IF NOT EXISTS reader_public_notes_book_chapter_idx ON reader_public_notes (book_id, chapter_id)`),
        db.prepare(`CREATE INDEX IF NOT EXISTS reader_public_notes_rotation_idx ON reader_public_notes (status, is_spoiler, is_pinned, approved_at, id)`),
        db.prepare(`CREATE TABLE IF NOT EXISTS chapter_emotions (
        visitor_key TEXT NOT NULL,
        book_id TEXT NOT NULL,
        chapter_id TEXT NOT NULL,
        emoji TEXT NOT NULL,
        ...
        db.prepare(`CREATE INDEX IF NOT EXISTS chapter_emotions_book_idx ON chapter_emotions (book_id, chapter_id)`),
        db.prepare(`CREATE TABLE IF NOT EXISTS reader_time_capsules (
        visitor_key TEXT NOT NULL,
        book_id TEXT NOT NULL,
        ...
        db.prepare(`CREATE INDEX IF NOT EXISTS community_posts_kind_created_idx ON community_posts (kind, created_at)`),
        db.prepare(`CREATE INDEX IF NOT EXISTS community_posts_book_created_idx ON community_posts (book_id, created_at)`),
        db.prepare(`CREATE TABLE IF NOT EXISTS community_votes (
      ...
      await addColumnIfMissing(db, 'books', names, 'country', `TEXT DEFAULT '' NOT NULL`);
      await addColumnIfMissing(db, 'books', names, 'publication_year', `INTEGER`);
      await addColumnIfMissing(db, 'books', names, 'page_count', `INTEGER DEFAULT 0 NOT NULL`);
      await addColumnIfMissing(db, 'books', names, 'planned_chapter_count', `INTEGER DEFAULT 0 NOT NULL`);
      await addColumnIfMissing(db, 'books', names, 'author_birthday', `TEXT DEFAULT '' NOT NULL`);
      await addColumnIfMissing(db, 'books', names, 'original_release_date', `TEXT DEFAULT '' NOT NULL`);
      await addColumnIfMissing(db, 'books', names, 'translator', `TEXT DEFAULT '' NOT NULL`);
      await addColumnIfMissing(db, 'books', names, 'editor', `TEXT DEFAULT '' NOT NULL`);
      await addColumnIfMissing(db, 'books', names, 'proofreader', `TEXT DEFAULT '' NOT NULL`);
      await addColumnIfMissing(db, 'books', names, 'playlist_url', `TEXT DEFAULT '' NOT NULL`);
      await addColumnIfMissing(db, 'books', names, 'team_pick', `INTEGER DEFAULT 0 NOT NULL`);
      await addColumnIfMissing(db, 'books', names, 'quote_of_day', `TEXT DEFAULT '' NOT NULL`);
      await addColumnIfMissing(db, 'books', names, 'search_aliases', `TEXT DEFAULT '[]' NOT NULL`);
      ...
      await addColumnIfMissing(db, 'books', names, 'suitability_profile', `TEXT DEFAULT '{}' NOT NULL`);
      await addColumnIfMissing(db, 'books', names, 'age_rating', `TEXT DEFAULT '' NOT NULL`);
      await addColumnIfMissing(db, 'books', names, 'age_reason', `TEXT DEFAULT '' NOT NULL`);
      ...
      await addColumnIfMissing(db, 'books', names, 'world_map_key', `TEXT`);
      await addColumnIfMissing(db, 'books', names, 'world_map_name', `TEXT DEFAULT '' NOT NULL`);
      await addColumnIfMissing(db, 'books', names, 'world_map_content_type', `TEXT DEFAULT '' NOT NULL`);
      await addColumnIfMissing(db, 'books', names, 'world_map_size_bytes', `INTEGER DEFAULT 0 NOT NULL`);
      await addColumnIfMissing(db, 'books', names, 'world_map_markers', `TEXT DEFAULT '[]' NOT NULL`);
      ...
      await db.prepare(`CREATE INDEX IF NOT EXISTS chapters_workflow_scheduled_idx ON chapters (workflow_status, scheduled_at)`).run();
      ...
      await db.prepare(`CREATE INDEX IF NOT EXISTS reader_library_book_status_idx ON reader_library (book_id, status, visitor_key)`).run();
      ...
      await addColumnIfMissing(db, 'comments', commentNames, 'context', `TEXT DEFAULT 'comments' NOT NULL`);
      ...
      await addColumnIfMissing(db, 'comments', commentNames, 'author_role', `TEXT DEFAULT 'reader' NOT NULL`);
      ...
      await db.prepare(`CREATE INDEX IF NOT EXISTS comments_book_context_status_created_idx ON comments (book_id, context, status, created_at)`).run();

      const profileColumns = await db.prepare(`PRAGMA table_info(reader_profiles)`).all();
      const profileNames = new Set((profileColumns.results || []).map((column) => column.name));
      await addColumnIfMissing(db, 'reader_profiles', profileNames, 'photo_key', `TEXT`);
      await addColumnIfMissing(db, 'reader_profiles', profileNames, 'photo_name', `TEXT DEFAULT '' NOT NULL`);
      await addColumnIfMissing(db, 'reader_profiles', profileNames, 'photo_content_type', `TEXT DEFAULT '' NOT NULL`);
      await addColumnIfMissing(db, 'reader_profiles', profileNames, 'photo_size_bytes', `INTEGER DEFAULT 0 NOT NULL`);
      await addColumnIfMissing(db, 'reader_profiles', profileNames, 'mascot_preferences', `TEXT DEFAULT '{}' NOT NULL`);

      const notificationColumns = await db.prepare(`PRAGMA table_info(notification_preferences)`).all();
      const notificationNames = new Set((notificationColumns.results || []).map((column) => column.name));
      await addColumnIfMissing(db, 'notification_preferences', notificationNames, 'comment_reply', `INTEGER DEFAULT 1 NOT NULL`);

      const readerNotificationColumns = await db.prepare(`PRAGMA table_info(reader_notifications)`).all();
      const readerNotificationNames = new Set((readerNotificationColumns.results || []).map((column) => column.name));
      await addColumnIfMissing(db, 'reader_notifications', readerNotificationNames, 'hidden_at', `TEXT`);
      await db.prepare(`CREATE INDEX IF NOT EXISTS reader_notifications_visitor_unread_idx ON reader_notifications (visitor_key, read_at, hidden_at)`).run();
    })().catch((error) => {
      schemaPromise = null;
      throw error;
    });
