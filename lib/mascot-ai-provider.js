...
function cleanVerifiedMessages(messages) {
  return (Array.isArray(messages) ? messages : [])
    .map((message) => ({
      character: message?.character === 'till' ? 'till' : message?.character === 'both' ? 'both' : 'ivan',
      ...
      messages: [
        {
          ...
          content: [
            ...
            'Ответь только JSON-объектом вида {"messages":[{"character":"ivan","text":"..."}]}.',
          ].join(' '),
        },
        {
          role: 'user',
