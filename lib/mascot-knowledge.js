...
export async function answerMascotQuestion({ question, askMode = 'both', bookSlug = '', visitorKey = '', currentChapter = 0, blockedTopics = [], allowSpoilers = false }) {
  ...
  const reply = (useful, ivanAfter = '', tillAfter = '', forceSensitive = sensitive) => ({
    messages: normalizeMascotLines(chooseMessages(mode, useful, ivanAfter, tillAfter, forceSensitive), { category: 'tip', sensitive: forceSensitive }),
  ...
  if (needsSpoilerConfirmation({ question: query, progressChapter, publishedChapterCount, allowSpoilers })) {
    return {
      messages: normalizeMascotLines(spoilerConfirmationMessages(mode === 'till' ? 'till' : 'ivan'), { category: 'tip', sensitive: true }),
  ...
  if (/подойд[её]т|романтик|откровен|темп|стекл|эмоцион/iu.test(normalized)) {
    const genres = book.genres?.length ? book.genres.join(', ') : 'не указаны';
    const tropes = book.tropes?.length ? book.tropes.join(', ') : 'не указаны';
    return reply(`Жанры: ${genres}. Тропы: ${tropes}. Для точного выбора также проверьте предупреждения и возрастное ограничение.`, 'Выбор всё равно остаётся за Вами.', 'Если там опасный герой и плохие решения, я уже заинтересован.');
  }
  ...
  return reply(overview, 'Если нужен точный ответ, спросите о тропах, предупреждениях, серии или знакомых персонажах.', 'А ещё можно спросить о жанрах. Я уже составил мнение.');
}
...
export function parseMascotDialogues(rows = []) {
  return rows.map((row) => ({
    ...
    lines: normalizeMascotLines(parseList(row.lines, []), { category: row.category || 'tip' }),
