...
export async function answerMascotQuestion({ question, askMode = 'both', bookSlug = '', visitorKey = '', currentChapter = 0 }) {
  const query = String(question || '').trim().slice(0, 1000);
  const normalized = query.toLocaleLowerCase('ru-RU');
  const mode = ['both', 'ivan', 'till'].includes(askMode) ? askMode : 'both';
  const sensitive = /селфхарм|самоповреж|суицид|насили|изнасил|утрат|травм|дискриминац|религи/iu.test(normalized);
