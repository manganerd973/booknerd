...
import { createChapterReaderNotifications } from './reader-notifications.js';
...
async function deliverToSubscriptions({ db, subscriptions, message, requestUrl }) {
...
export async function sendTestNotification({ visitorKey, endpoint = '', requestUrl }) {
  const db = await ensureDb();
  const statement = endpoint
    ? db.prepare(
      `SELECT endpoint, visitor_key, p256dh, auth
       FROM push_subscriptions
       WHERE visitor_key = ? AND endpoint = ?
       ORDER BY updated_at DESC`
    ).bind(visitorKey, endpoint)
    : db.prepare(
      `SELECT endpoint, visitor_key, p256dh, auth
       FROM push_subscriptions
       WHERE visitor_key = ?
  ...
  return deliverToSubscriptions({
    ...
    message: {
      title: 'BOOKNERD ✦',
      body: 'Проверка пройдена: уведомления работают.',
...
const GLOBAL_NOTIFICATION_KEY = 'booknerd-settings';
...
export async function notifyBookPreferenceEvent({
  ...
  const defaultEnabled = preference === 'newChapter' || preference === 'commentReply';
  const preferenceMatch = sameAuthor
    ? `EXISTS (
         ...
         WHERE p.visitor_key = s.visitor_key
           AND p.book_key = '${GLOBAL_NOTIFICATION_KEY}'
           AND p.${column} = 1
       )`
    : `EXISTS (
         SELECT 1 FROM notification_preferences p
         ...
         NOT EXISTS (
           SELECT 1 FROM notification_preferences p
           WHERE p.visitor_key = s.visitor_key AND p.book_key IN (?, ?)
         )
         AND EXISTS (
           SELECT 1 FROM notification_preferences p
           WHERE p.visitor_key = s.visitor_key
         ...
         AND NOT EXISTS (
           SELECT 1 FROM notification_preferences p
           WHERE p.visitor_key = s.visitor_key AND p.book_key = '${GLOBAL_NOTIFICATION_KEY}'
         )
       )` : ''}`;
  ...
  const bindings = sameAuthor
    ? [book.author]
    : defaultEnabled
      ? [book.slug, book.id, book.slug, book.id, book.slug, book.id]
      : [book.slug, book.id, book.slug, book.id];
...
export async function notifyPublishedBook({ bookId, requestUrl }) {
  const db = await ensureDb();
  const book = await db.prepare(
    `SELECT id, slug, title, author FROM books WHERE id = ? AND published = 1 LIMIT 1`
  ).bind(bookId).first();
  if (!book) return { sent: 0 };
  ...
  const vapid = { subject: origin, publicKey: keys.publicKey, privateKey: keys.privateKey };
  const message = {
    title: 'Новая книга в BOOKNERD ✦',
    body: `«${book.title}» — ${book.author}. Уже можно открыть в библиотеке.`,
    url: `/books/${book.slug}`,
    icon: '/booknerd-icon-v2-192.png',
    badge: '/booknerd-icon-v2-192.png',
    topic: `new-book-${book.id.slice(0, 18)}`,
    bookId: book.id,
  };
  let sent = 0;
  for (let index = 0; index < subscriptions.length; index += 25) {
...
export async function notifyPublishedChapter({ chapterId, requestUrl }) {
  ...
  const inboxCreated = await createChapterReaderNotifications({ db, chapterId: chapter.id });

  const claimed = await db.prepare(
    `INSERT OR IGNORE INTO push_announcements (chapter_id, sent_at) VALUES (?, ?)`
  ).bind(chapter.id, new Date().toISOString()).run();
  if (!Number(claimed.meta?.changes || 0)) return { sent: 0, inboxCreated, duplicate: true };
  ...
  const subscriptionsResult = await db.prepare(
    `SELECT s.endpoint, s.visitor_key, s.p256dh, s.auth
     ...
     ORDER BY s.updated_at DESC`
  ).bind(chapter.slug, chapter.book_id, chapter.slug, chapter.book_id, chapter.slug, chapter.book_id).all();
  const subscriptions = subscriptionsResult.results || [];
  if (!subscriptions.length) {
    await db.prepare(`DELETE FROM push_announcements WHERE chapter_id = ?`).bind(chapter.id).run();
    return { sent: 0, inboxCreated, noSubscribers: true };
  }
  const message = {
    title: chapter.book_title,
    body: chapter.chapter_title || `Глава ${chapter.chapter_number}`,
  ...
  const result = await deliverToSubscriptions({ db, subscriptions, message, requestUrl });
  if (!result.sent) {
    // A temporary push-provider failure must not permanently block the next
    // retry for this chapter.
    await db.prepare(`DELETE FROM push_announcements WHERE chapter_id = ?`).bind(chapter.id).run();
  }
  return { ...result, inboxCreated };
}
