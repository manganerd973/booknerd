...
function defaultQuotePool() {
  return EDITORIAL_QUOTES.map((quote, index) => ({
    ...defaultQuoteForSlot(index),
...
export async function getQuotePool() {
  let db;
  ...
  try {
    ...
    const result = await db.prepare(`${baseQuery} ORDER BY n.is_pinned DESC, n.approved_at ASC, n.id ASC LIMIT 120`).all();
  ...
  if (approvedReaderQuotes.length) return approvedReaderQuotes;
  ...
  if (uniqueCandidates.length < 2) {
    uniqueCandidates.push(...defaultQuotePool());
  }

  return uniqueQuotes(uniqueCandidates);
}
...
export async function getQuoteOfDay(now = new Date()) {
  ...
  const quotePool = await getQuotePool();
