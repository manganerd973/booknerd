...
export const NOTIFICATION_TYPES = new Set([
  'new_chapter', 'comment_reply', 'comment_upvote', 'reader_level', 'achievement', 'nomination', 'month_summary',
]);
    ...
    db.prepare(
      `INSERT OR IGNORE INTO reader_notifications
       ...
       SELECT parent.visitor_key || ':reply:' || reply.id,
              ...
              CASE
                WHEN reply.author_role = 'admin' AND reply.chapter_id IS NOT NULL
                  THEN 'Администратор BOOKNERD ответил вам в «' || b.title || '», глава ' || COALESCE(ch.chapter_number, '') || '.'
                WHEN reply.author_role = 'admin'
                  THEN 'Администратор BOOKNERD ответил вам в обсуждении «' || b.title || '».'
                WHEN reply.chapter_id IS NOT NULL
              ...
              CASE WHEN reply.chapter_id IS NOT NULL
                ...
                WHEN reply.context = 'discussion'
                THEN '/books/' || b.slug || '#discussion-comment-' || reply.id
      ...
      `INSERT OR IGNORE INTO reader_notifications
       ...
       SELECT c.visitor_key || ':comment-upvote:' || c.id || ':' || v.voter_key,
              ...
              CASE WHEN c.chapter_id IS NOT NULL
                ...
                WHEN c.context = 'discussion'
                THEN '/books/' || b.slug || '#discussion-comment-' || c.id
