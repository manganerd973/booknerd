...
export async function listPopularComments(limit = 6) {
  ...
  return cachedRead(`popular-comments:${safeLimit}`, 60_000, async () => {
    const result = await db.prepare(
    `SELECT c.id, c.author_role, c.author_name, c.body, c.is_spoiler, c.created_at, c.chapter_id, c.context,
