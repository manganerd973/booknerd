...
export async function GET(request) {
  ...
  try {
    ...
    return new Response(JSON.stringify({ format: 'BOOKNERD_BACKUP_V39', createdAt: new Date().toISOString(), createdBy: auth.email || '', data }), { headers: { 'content-type': 'application/json; charset=utf-8', 'content-disposition': `attachment; filename="booknerd-backup-${new Date().toISOString().slice(0, 10)}.json"`, 'cache-control': 'no-store' } });
